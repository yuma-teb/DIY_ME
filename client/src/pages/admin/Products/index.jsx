...
const Index = () => {
  ...
  const createData = (id, name, price, stock, sold, rating, DB, isActive) => {
    return { id, name, price, stock, sold, rating, DB, isActive };
  };
  const createDataFromProduct = (product, index) => {
    const { name, price, stock, sold, ratingAverage, _id, isActive } = product;
    return createData(index + 1, name, price, stock, sold, ratingAverage, _id, isActive);
  };
                        ...
                        {columns.map((column) => {
                          ...
                          return (
                            <TableCell key={column.id} align={column.align}>
                              {column.id === 'button' ? (
                                <div>
                                  <Link to={`/admin/products/${row.DB}`}>
                                    <IconButton>
                                      <ViewIcon color="view" />
                                    </IconButton>
                                  </Link>
                                  <Link to={`/admin/products/${row.DB}/edit`}>
                                    <IconButton>
